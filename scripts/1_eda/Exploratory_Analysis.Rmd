...
libraries_to_install <- c("tidyverse", "ggplot2", "writexl", "openxlsx", "shiny", "kableExtra", "broom", "here", "TOC", "psych", "lme4", "lmerTest", "broom", "nnet")
...
Выберем нужное для презентации
...
#### Следующий этап. Модели оценки когнитивных функций.

В смешанных моделях в качестве случайного эффекта выбран id пациента.
Используем lmer из пакета lmerTest, чтобы оценить p-value.
...
model_1_1_2 <- lm(ZVM ~ `head rotation` + `Total score PANSS` + `THF dose`, data = selected_data_ZVM)

model_1_2_1 <- lm(ZMT ~ `Total score PANSS` + `THF dose`, data = selected_data_ZMT)
model_1_2_2 <- lm(ZMT ~ akathisia + `Total score PANSS` + `THF dose`, data = selected_data_ZMT)
...
model_1_4_2 <- lm(ZVF ~ tremor + `Total score PANSS` + `THF dose`, data = selected_data_ZVF)
...
Разбиваем по визитам

```{r}
model_ZVM_visit_1 <- lm(ZVM ~ `head rotation` + `Total score PANSS` + `THF dose`, data = selected_data_ZVM %>% filter(visit==1))
model_ZVM_visit_2 <- lm(ZVM ~ `head rotation` + `Total score PANSS` + `THF dose`, data = selected_data_ZVM %>% filter(visit==2))

model_ZMT_1_visit_1 <- lm(ZMT ~ akathisia + `Total score PANSS` + `THF dose`, data = selected_data_ZMT %>% filter(visit==1))
model_ZMT_1_visit_2 <- lm(ZMT ~ akathisia + `Total score PANSS` + `THF dose`, data = selected_data_ZMT %>% filter(visit==2))

model_ZMT_2_visit_1 <- lm(ZMT ~ `Total score SAS` + `Total score PANSS` + `THF dose`, data = selected_data_ZMT %>% filter(visit==1)) 
model_ZMT_2_visit_2 <- lm(ZMT ~ `Total score SAS` + `Total score PANSS` + `THF dose`, data = selected_data_ZMT %>% filter(visit==2)) 

model_ZVF_visit_1 <- lm(ZVF ~ tremor + `Total score PANSS` + `THF dose`, data = selected_data_ZVF %>% filter(visit==1))
model_ZVF_visit_2 <- lm(ZVF ~ tremor + `Total score PANSS` + `THF dose`, data = selected_data_ZVF %>% filter(visit==2))

model_ZToL_visit_1 <- lm(ZToL ~ `Total score SAS` + `Total score PANSS` + `THF dose`, data = selected_data_ZToL %>% filter(visit==1)) 
model_ZToL_visit_2 <- lm(ZToL ~ `Total score SAS` + `Total score PANSS` + `THF dose`, data = selected_data_ZToL %>% filter(visit==1))
...
#### Модели ЭПС логистическая регрессия
...
glm(head_rotation_bin ~ CPZE + `Total score PANSS`, family = binomial, data = data_for_logr) %>% summary()
glm(tremor_bin ~ CPZE + `Total score PANSS`, family = binomial, data = data_for_logr) %>% summary()
glm(akathisia_bin ~ CPZE + `Total score PANSS`, family = binomial, data = data_for_logr) %>% summary()
lm(`Total score SAS` ~ CPZE + `Total score PANSS`, data = data_filtered) %>% summary()
```
...
# Объединяем и округляем результаты в одну таблицу
