...
libraries_to_install <- c("tidyverse", "ggplot2", "writexl", "openxlsx", "shiny", "kableExtra", "broom", "here", "TOC", "psych")
...
```{r}
data_filtered %>% 
  select("id", "gender", "age", "disease duration", "antipsychotic", "antipsychotic dose", "antipsychotic generation", "THF dose", "course", "education", "smoke", "visit", "Total score SAS", "Total score PANSS", "Comp Z", "CPZE") %>% 
  describe() %>% 
  kable("html") %>%
...
```
```{r}
rez <- cor.test(data_filtered$age, data_filtered$`Total score PANSS`, method = "spearman")
rez %>% str()

cor = cor(data_filtered$age, data_filtered$`Total score PANSS`, method = "spearman")
p_value = cor.test(data_filtered$age, data_filtered$`Total score PANSS`, method = "spearman")$p.value
ci_low = spearman.ci(data_filtered$age, data_filtered$`Total score PANSS`)$conf.int[1] %>% as.numeric()
ci_high = spearman.ci(data_filtered$age, data_filtered$`Total score PANSS`)$conf.int[2] %>% as.numeric()
ci_high
```
...
source(path)
lm_shiny(data_filtered %>% select(!matches("P\\d{1}") & !matches("N\\d{1}") & !matches("G\\d{1}")))
```
...
path <- here("scripts/1_eda", "lm_shiny.R")
source(path)
lm_shiny(data_filtered %>% filter(visit==1) %>% select(!matches("P\\d{1}") & !matches("N\\d{1}") & !matches("G\\d{1}")))
```
...
path <- here("scripts/1_eda", "lm_shiny.R")
source(path)
lm_shiny(data_filtered %>% filter(visit==2) %>% select(!matches("P\\d{1}") & !matches("N\\d{1}") & !matches("G\\d{1}")))
```
...
path <- here("scripts/1_eda", "lm_shiny.R")
source(path)
lm_shiny(difference_df %>% select(!matches("P\\d{1}") & !matches("N\\d{1}") & !matches("G\\d{1}")))
```