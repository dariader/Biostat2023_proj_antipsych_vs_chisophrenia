...
libraries_to_install <- c("tidyverse", "ggplot2", "writexl", "openxlsx", "shiny", "kableExtra", "broom", "here", "TOC", "psych", "lme4", "lmerTest")
...
selected_data <- data_filtered %>% 
  select(-excitment, -cognitive, -depression , 
         -starts_with('N'), -starts_with('P'), -starts_with('G'), -starts_with('Z'), ZVM, gender) %>% 
  select(-`Verbal Memory`, -`Digit Sequencing`, 
         -`Token Motor Task`, -`Verbal Fluency`, -`Symbol Coding`, -`Tower of London`) %>% 
  select(-`Comp Z`)

mixed_model <- lmer(ZVM ~ `Total score SAS` + (1 | id), data = selected_data, REML=F)
summary(mixed_model)
```
...
```{r}
# Для предсказания ZVM
selected_data_ZVM <- data_filtered %>% 
  select(-excitment, -cognitive, -depression , 
         -starts_with('N'), -starts_with('P'), -starts_with('G'), -starts_with('Z'), ZVM, gender, gender, gait, `glabella tap`) %>% 
  select(-`Verbal Memory`, -`Digit Sequencing`, 
         -`Token Motor Task`, -`Verbal Fluency`, -`Symbol Coding`, -`Tower of London`) %>% 
  select(-`Comp Z`)

# Для предсказания ZMT
