...
Исследуем изменение количественных переменных между визитами используя t.test ($\alpha = 0.05$). Укажем значимость в significant_flag. 
Перед этим проведем shapiro.test ($\alpha = 0.05$).
...
t_test_results <- lapply(selected_variables, function(variable) {
  ...
  t_test_result <- t.test(data_filtered[data_filtered$visit == "1", variable],
                           data_filtered[data_filtered$visit == "2", variable])
  ...
  result_df <- data.frame(
    ...
    significant_flag = ifelse(t_test_result$p.value < alpha, "1", "0"),
...
significant_variables <- t_test_results %>% 
  filter(significant_flag == 1) %>% 
  select(-significant_flag)
...
ggplot(significant_results, aes(x = Variable, y = p_value)) +
  geom_bar(stat = "identity") +
  geom_hline(yintercept = alpha, linetype = "dashed", color = "green") +  # Добавление горизонтальной линии для уровня значимости
  labs(title = "Significance of Variables between Visit 1 and Visit 2",
       x = "Variable",
       y = "p-value") +
  theme_minimal() +
...
Ладно, не будем сравнивать p-value, просто отметим эти значимые переменные.
...
```{r}
ggplot(difference_df, aes(x = id, y = `Total score SAS`)) +
  geom_point() +
...
```{r}
ggplot(difference_df, aes(x = `Total score SAS`)) +
